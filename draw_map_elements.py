...
from plugin.datasets.data_osm.rasterize import preprocess_map, preprocess_osm_map
from plugin.datasets.data_osm.const import CAMS, NUM_CLASSES, IMG_ORIGIN_H, IMG_ORIGIN_W
from plugin.datasets.data_osm.vector_map import VectorizedLocalMap
from plugin.datasets.data_osm.lidar import get_lidar_data
from plugin.datasets.data_osm.image import normalize_img, img_transform
from plugin.datasets.data_osm.utils import label_onehot_encoding
from plugin.datasets.nusc_sdmap_dataset import PMapNetDataset
...
def get_vectors(rec, nusc, vector_map):
    ...
    vectors, polygon_geom, osm_vectors = vector_map.gen_vectorized_samples(location, ego_pose['translation'], ego_pose['rotation'])
    
    return vectors, polygon_geom, osm_vectors
    
# todo
def get_semantic_map(rec, nusc, vector_map):
    time = rec['timestamp']
    scene_token = rec['scene_token']
    scene_id = nusc.get('scene', scene_token)['name']

    vectors, _, osm_vectors = get_vectors(rec, nusc, vector_map)
    patch_size = (60, 30)
    canvas_size = (100, 50)
    osm_masks, _ = preprocess_osm_map(osm_vectors, patch_size, canvas_size)

    mask_flag = False
    mask_ratio = -1 # random ratio
    mask_patch_h = 20
    mask_patch_w = 20
    mask_config = [mask_flag, mask_ratio, mask_patch_h, mask_patch_w]

    thickness=5
    angle_class=36

    instance_masks, forward_masks, backward_masks, instance_mask_map = preprocess_map(
        mask_config, vectors, patch_size, canvas_size, 
        NUM_CLASSES, thickness, angle_class)
    ...
    forward_oh_masks = label_onehot_encoding(forward_masks, angle_class+1)
    backward_oh_masks = label_onehot_encoding(backward_masks, angle_class+1)
...
def create_nuscenes_infos_map(root_path,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        patch_size = (60, 30)
        canvas_size = (100, 50)
        sd_map_path = 'plugin/datasets/data_osm/osm'
        vector_map = VectorizedLocalMap(root_path, patch_size=patch_size, canvas_size=canvas_size, sd_map_path=sd_map_path)
        semantic_masks, instance_masks, _, _, direction_masks, osm_masks, osm_vectors, masked_map = get_semantic_map(sample, nusc, vector_map)


        # import pdb
        # pdb.set_trace()
        #这里就是每一帧的车道线，只是我把它们用matrix叠起来了，循环一次读到这里就是独立的一帧,下面是画图代码
        element_num = len(map_matrix)-element_num0
        plt.figure(figsize=(8, 6))

        # 子图 1
        plt.subplot(2, 1, 1)  # 2 行 1 列，第 1 个子图
        plt.title("map_matrix")
        # plt.figure()
        for i in range(element_num):
            name = str(i)
            pts = map_matrix[i].reshape(-1,2)
        ...
        plt.title("sd map")
        # plt.figure()
        for i in range(len(osm_vectors)):
            name = str(i)
            pts = osm_vectors[i][0]
            plt.plot(pts[:, 0], pts[:, 1], label=name)
            plt.legend()

        plt.savefig('vis_sdmap.png')
        plt.close()
